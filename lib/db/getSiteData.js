...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
    ...
    const block = pageRecordMap.block || {}
        const rawMetadata = getBlockValue(block[pageId])
    console.error('[DEBUG-TYPE]', 'pageId:', pageId, 'type:', rawMetadata?.type, 'blockKeysCount:', Object.keys(block).length); console.error('[DEBUG-TYPE2]', 'hasKey:', Object.prototype.hasOwnProperty.call(block, pageId), 'entryType:', typeof block[pageId], 'entryKeys:', JSON.stringify(Object.keys(block[pageId]||{})), 'entryJSON:', JSON.stringify(block[pageId]).slice(0,600))
    if (
    ...
    const collection = getBlockValue(Object.values(pageRecordMap.collection)[0]) || {}
