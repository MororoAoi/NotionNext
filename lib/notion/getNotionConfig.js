 ...
 *
 */
import { getDateValue, getTextContent, getBlockValue } from 'notion-utils'
...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  let content = getBlockValue(pageRecordMap.block[configPageId])?.content
  for (const table of ['Config-Table', 'CONFIG-TABLE']) {
    if (content) break
    pageRecordMap = await getPostBlocks(configPageId, table)
    content = getBlockValue(pageRecordMap.block[configPageId])?.content
  }
  ...
  //     console.log('内容', contentId, configgetBlockValue(pageRecordMap.block[contentId])?.type === 'collection_view')
  //   }
  const configTableId = content?.find(contentId => {
    return getBlockValue(pageRecordMap.block[contentId])?.type === 'collection_view'
  })
  ...
  const rawMetadata = getBlockValue(databaseRecordMap)
  ...
  const collection = getBlockValue(pageRecordMap.collection[collectionId])
