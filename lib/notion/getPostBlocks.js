...
const pageData = await api.getPage(id, { gotOptions: { hooks: { beforeRequest: [options => { if (options.url && options.url.pathname && options.url.pathname.includes('/api/v3/syncRecordValues')) { options.url.pathname = options.url.pathname.replace('/api/v3/syncRecordValues', '/api/v3/syncRecordValuesMain') } }] } } })      const end = new Date().getTime()
